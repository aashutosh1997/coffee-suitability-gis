...
| **R-CLIMSYNTH** | DEM-derived synthetic climate cannot encode real stressors (management, irregular rainfall, microclimate); validating against it under-detects climate-driven disagreements. | M | M | Real CHELSA/WorldClim path is documented (ADR-0007) and is the production source on first real-data validation; the synthetic generator is fixture-only with a loud range-assert; this limitation is also called out in the validation report. | Data Engineer |
